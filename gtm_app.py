...
st.set_page_config(
    page_title="Metabase · GTM Toolkit",
    page_icon="📊",
    layout="centered"
)
...
st.markdown("""
<style>
    /* Metabase: dark navy sidebar, white main, blue accent #509EE3 */
    ...
    /* Primary / submit button → Metabase blue */
    ...
    .stTextInput input:focus {
        ...
        box-shadow: 0 0 0 1px #509EE3 !important;
    }
...
for d in search_dirs:
    for name in ["metabase_logo.png", "logo.png", "logo.jpg", "logo.jpeg"]:
...
if logo_file:
    ...
    st.sidebar.markdown(
        f'<img src="data:{mime};base64,{encoded}" width="140" style="margin-bottom:8px;">',
        unsafe_allow_html=True
    )
else:
    st.sidebar.markdown("## 📊")

st.sidebar.markdown("### Metabase")
st.sidebar.markdown("<span style='color:#A8B4C8; font-size:0.8rem;'>GTM Toolkit · Internal use only</span>", unsafe_allow_html=True)
st.sidebar.divider()
...
st.sidebar.divider()
st.sidebar.markdown("<span style='color:#A8B4C8; font-size:0.75rem;'>Powered by Claude AI</span>", unsafe_allow_html=True)
st.sidebar.markdown("<span style='color:#A8B4C8; font-size:0.75rem;'>© Metabase · Confidential</span>", unsafe_allow_html=True)
...
if "Pre-Call" in tool:
    ...
    with st.form("brief_form"):
        ...
        website_url  = st.text_input("Website URL",    placeholder="e.g. https://notion.so")

        col1, col2 = st.columns(2)
        with col1:
            contact_name  = st.text_input("Contact name",  placeholder="e.g. Jane Smith")
        with col2:
            contact_title = st.text_input("Contact title", placeholder="e.g. Head of Data")
...
# ─── Tool: Cold Email Writer ──────────────────────────────────────────────────
    ...
    with st.form("email_form"):
        company_name   = st.text_input("Company name *",   placeholder="e.g. Intercom")
        website_url    = st.text_input("Website URL",       placeholder="e.g. https://intercom.com")

        col1, col2 = st.columns(2)
        with col1:
            contact_name  = st.text_input("Contact name",  placeholder="e.g. Jane Smith")
        with col2:
            contact_title = st.text_input("Contact title", placeholder="e.g. VP of Product")

        recipient_email = st.text_input("Recipient email (optional)", placeholder="e.g. jane@intercom.com")

        custom_notes = st.text_area(
            "Additional context (optional)",
            placeholder="e.g. They just raised a Series C, recently posted about scaling their analytics, building a customer portal, or we share a mutual connection...",
            height=100
        )
    ...
    if submitted:
        if not company_name.strip():
            ...
            if score_match:
                ...
                colours = {1: "#EF4444", 2: "#F97316", 3: "#509EE3", 4: "#22C55E"}
